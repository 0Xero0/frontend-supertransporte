...
export class ServicioArchivosEncuestas extends Autenticable {
  ...
  guardarEvidencia(archivo: File, idVigilado: string, extensionValida: string) {
    const endpoint = '/api/v1/archivos/evidencias'
    const formData = new FormData()

    formData.append('archivo', archivo)
     formData.append('idVigilado', idVigilado.toString())
    formData.append('rutaRaiz', 'evidencias')
    formData.append('extension', extensionValida)
    return this.http.post<ArchivoTemporal>(
      `${this.host}${endpoint}`,
      formData,
      { headers: { Authorization: `Bearer d4a32a3b-def6-4cc2-8f77-904a67360b53` } }
